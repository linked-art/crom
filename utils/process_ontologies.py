...
def process_classes(dom):
    ...
    for c in classes:
        ...
        try:
            ...
            # could be overlay between files (e.g. crm:Acquisition subClassOf la:Transfer)
            overlay = True
        ...
        if name in seen and overlay:
            for s in stuff:
                if s[0] == name:
                    # NOTE: can't have the same class multiple times in class hierarchy
                    # otherwise we get MRO failures as python cannot determine the right
                    # parent class resolution order
                    s[5] = subCls
                    break
        else:
            seen[name] = 1
